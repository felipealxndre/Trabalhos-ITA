...
% A Cl_beta é conhecida como efeito dietro
...
for i_cond=1:3
    ...
    x_eq = fsolve(@trim_function_Ex8,x_eq_0,options,trim_par,i_cond);
    [~,X_eq,U_eq,Y_eq] = trim_function_Ex8(x_eq,trim_par,i_cond);
...
save trim_output_Ex8.mat trim_output
...
for i_cond=1:3
    ...
    for j=1:nX
        dX = zeros(nX,1);
        dX(j) = delta_val;
        [Xdot_plus,Y_plus] = dynamics_Ex8(0,X_eq+dX,U_eq,i_cond);
        [Xdot_minus,Y_minus] = dynamics_Ex8(0,X_eq-dX,U_eq,i_cond);
    ...
    for j=1:nU
        dU = zeros(nU,1);
        dU(j) = delta_val;
        [Xdot_plus,Y_plus] = dynamics_Ex8(0,X_eq,U_eq+dU,i_cond);
        [Xdot_minus,Y_minus] = dynamics_Ex8(0,X_eq,U_eq-dU,i_cond);
...
save lin_output_Ex8.mat lin_output
...
%--------------------------------------------------------------------------
% Ex. 8) Simulation of disturbed initial condition:
...
t_f = 500;
T = 0:dt:t_f;
[X,Y] = ode4xy(@simulate_flight_Ex8,T,X0,U0,i_cond);
U = Y(:,end-4:end);
Y = Y(:,1:end-5);
