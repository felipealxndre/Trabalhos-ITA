...
% 
% X0 = trim_output(i_cond).X_eq;
% X0(7) = X0(7) + 2; %perturbação em beta
% U0 = trim_output(i_cond).U_eq;
% 
% dt = 0.010;
% t_f = 20;
...
%--------------------------------------------------------------------------
%  Simulation of response to a elevator doublet:
...
t_f = 20;
T = 0:dt:t_f;
[X,Y] = ode4xy(@simulate_flight_Ex3,T,X0,U0,i_cond);
U = Y(:,end-4:end);
Y = Y(:,1:end-5);
...
k_alpha = K(1);
k_q = K(2);
alpha_deg_eq = trim_output(i_cond).X_eq(2);
q_deg_s_eq = trim_output(i_cond).X_eq(3);

aircraft.SAS = struct('k_alpha', k_alpha, 'k_q', k_q, ...
    'alpha_deg_eq', alpha_deg_eq, 'q_deg_s_eq', q_deg_s_eq);

dt = 0.010;
t_f = 20;
T = 0:dt:t_f;
[X,Y] = ode4xy(@simulate_flight_Ex3_SAS,T,X0,U0,i_cond);
Delta_delta_e_deg_SAS = Y(:, end);
U = Y(:,end-5:end-1);
Y = Y(:,1:end-6);
...
figure
plot(T, U(:, 3) - Delta_delta_e_deg_SAS)
hold on
plot(T, Delta_delta_e_deg_SAS)
plot(T, U(:, 3))
grid on
xlabel('t [s]')
ylabel('Elevator [deg]')
legend('Pilot', 'SAS', 'Total')
