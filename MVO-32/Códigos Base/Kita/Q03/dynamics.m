function [Xdot,Y] = dynamics(t,X,U,i_cond)
% Modificado para Lista 3:
% 1. Assinatura da função atualizada (com i_cond)
% 2. Índices de controle U atualizados (6 controles)
% 3. Chamadas de função atualizadas (com i_cond)
% 4. Vetor de saída Y atualizado (12 saídas)
...
% --- Cinemática Rotacional (Matrizes) ---
C_psi = DCM(3, deg2rad(psi_deg));
C_theta = DCM(2, deg2rad(theta_deg));
C_phi = DCM(1, deg2rad(phi_deg));

C_bv = C_phi*C_theta*C_psi; % Vento para Corpo
C_vb = transpose(C_bv); % Corpo para Vento

C_mbeta = DCM(3, -deg2rad(beta_deg));
C_alpha = DCM(2, deg2rad(alpha_deg));

C_ba = C_alpha*C_mbeta; % Aerodinâmico para Corpo
C_ab = transpose(C_ba); % Corpo para Aerodinâmico
...
% CORREÇÃO: Passando i_cond para aero_loads
[F_aero_b, M_aero_C_b] = aero_loads(X,U,i_cond);
[F_prop_b, M_prop_C_b, T1, T2] = prop_loads(X,U); % Saída atualizada
...
pdot_deg_s_2 = rad2deg(pdot_rad_s_2);
qdot_deg_s_2 = rad2deg(qdot_rad_s_2);
rdot_deg_s_2 = rad2deg(rdot_rad_s_2);

% --- Derivadas dos Estados Cinemáticos ---
...
V_i = C_vb*V_b;
xdot = V_i(1); ydot = V_i(2); zdot = V_i(3);
hdot = -zdot;
...
% --- Vetor de Saída Y ---
% CORREÇÃO: Passando i_cond para aero_databank
...
% Vetor Y atualizado para 12 elementos
Y = [
    gamma_deg
    T1 % Novo
    T2 % Novo
    Mach % Índice antigo 3
    CD % Índice antigo 4
    CL % Índice antigo 5
    Cm % Índice antigo 6
    CY % Índice antigo 7
    Cl % Índice antigo 8
    Cn % Índice antigo 9
    rho % Índice antigo 10
    q_bar % Índice antigo 11
    ];
