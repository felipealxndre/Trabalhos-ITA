...
% Date: 2022-03-07
%   -> 2022 version: updated the output equation and enabled (with a flag)
%      printing time stamps on the command window
...
% Set to 1 if you would like to print the time stamps on the command window
print_time_stamps = 0;

X(:,1) = x0;
Y(:,1) = y0;

if print_time_stamps==1, fprintf('\nt = %8.3f s\n',tspan(1)); end
...
for i = 2:N
  ...
  f2 = feval(odefun, ti+0.5*hi, xi+0.5*hi*f1, varargin{:});
  f3 = feval(odefun, ti+0.5*hi, xi+0.5*hi*f2, varargin{:});  
  f4 = feval(odefun, tspan(i),  xi+hi*f3,     varargin{:});
  X(:,i) = xi + (hi/6)*(f1 + 2*f2 + 2*f3 + f4);
  Y(:,i-1) = y1;
  if print_time_stamps==1, fprintf('t = %8.3f s\n',tspan(i)); end
end

tf = tspan(N);
xf = X(:,N);
[~,yf] = feval(odefun, tf, xf, varargin{:});
Y(:,N) = yf;
